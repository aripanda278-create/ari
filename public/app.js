...
async function googleLogin(){avatar=$("avatarInput").value.trim();lang=$("languageInput").value;if(!avatar)return $("gateStatus").textContent=lang==="en"?"Enter your avatar name first.":"Prvo upiši avatar ime.";localStorage.setItem("mhq_avatar",avatar);localStorage.setItem("mhq_lang",lang);$("gateStatus").textContent=lang==="en"?"Opening Google sign-in…":"Otvaram Google prijavu…";try{const provider=new GoogleAuthProvider();provider.setCustomParameters({prompt:"select_account"});await signInWithPopup(auth,provider)}catch(error){console.error(error);const code=error?.code||"unknown";$("gateStatus").textContent=(lang==="en"?"Google sign-in failed":"Google prijava nije uspjela")+` (${code}).`}}
